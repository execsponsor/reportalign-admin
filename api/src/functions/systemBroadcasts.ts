...
async function listBroadcasts(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    if (status === 'active') whereClause += ` AND sb.is_active = true AND (sb.ends_at IS NULL OR sb.ends_at > NOW()) AND (sb.starts_at IS NULL OR sb.starts_at <= NOW())`;
    else if (status === 'scheduled') whereClause += ` AND sb.is_active = true AND sb.starts_at > NOW()`;
    else if (status === 'expired') whereClause += ` AND (sb.is_active = false OR (sb.ends_at IS NOT NULL AND sb.ends_at <= NOW()))`;
    ...
    return { status: 200, jsonBody: { success: true, data: { broadcasts: snakeToCamel(result.rows) } } };
  } catch (err) {
...
async function updateBroadcast(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    const body = (await req.json()) as Record<string, unknown>;
    const pool = getPool();
    const updates: string[] = []; const params: unknown[] = []; let paramIdx = 1;
    for (const field of ['title', 'message', 'link_url', 'link_text', 'broadcast_type', 'starts_at', 'ends_at', 'is_active', 'show_on_auth_pages', 'show_on_app']) {
      if (body[field] !== undefined) { updates.push(`${field} = $${paramIdx++}`); params.push(body[field]); }
    }
...
async function listMaintenanceWindows(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    return { status: 200, jsonBody: { success: true, data: { windows: snakeToCamel(result.rows) } } };
  } catch (err) {
...
async function createMaintenanceWindow(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    const validReasons = ['scheduled_maintenance', 'security_update', 'database_migration', 'infrastructure_upgrade', 'emergency_maintenance', 'other'];
...
async function updateMaintenanceWindow(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    const pool = getPool();
    const updates: string[] = []; const params: unknown[] = []; let paramIdx = 1;
    for (const field of ['title', 'description', 'reason', 'scheduled_start', 'scheduled_end', 'actual_start', 'actual_end', 'status']) {
      if (body[field] !== undefined) { updates.push(`${field} = $${paramIdx++}`); params.push(body[field]); }
    }
    if (updates.length === 0) return { status: 400, jsonBody: { success: false, error: 'No fields to update' } };
    updates.push(`updated_at = NOW()`);
